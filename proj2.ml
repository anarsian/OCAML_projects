...
let rec exprToString e = match e with
  ...
  | Sinfun(a,b,c) -> "sin(pi*"^exprToString a^"*pi*"^exprToString b^"*pi*"^exprToString c^")"
  |Other(a,b) -> exprToString a^"*sin(pi*"^exprToString b^")"
...
let buildSinfun(e1,e2,e3)          = Sinfun(e1,e2,e3)
let buildOther(e1,e2)              = Other(e1,e2)
...
let rec eval (e,x,y) = match e with
  ...
  | Sinfun(a,b,c) -> sin(pi*.eval(a,x,y)*.pi*.eval(b,x,y)*.pi*.eval(c,x,y))
  | Other(a,b) -> sin(pi*.eval(a,x,y))*.eval(b,x,y)
...
let rec build (rand, depth) = 
  if(depth = 0)
  then if(rand(1,3)=1) then buildX()
    else buildY()
  else
    let rand1 = rand(1,10) in
      match rand1 with 
        |1 -> build (rand, depth-1)
        |2 -> build(rand, depth-1)
        ...
        | 8 -> buildSinfun(build(rand,depth-1), build(rand,depth-1), build(rand,depth-1))
        | 9 -> buildOther(build(rand,depth-1), build(rand,depth-1))
        | _ -> failwith "yooooooo"
;;

let doRandomGray = (9,17,25)
let doRandomColor = (9,17,25);;
