...
let pipe fs = 
  let f a x =  fun fu -> x( a fu) in 
  let base = fun fuu ->fuu in
...
let rec clone = (*fun x -> fun n -> 
                  if(n=0) then
                  [] else
                  x::clone x (n-1)*)
  (*fun b ->*)
  (************CHECK IF METHOD SIGNATURE CAN BE CHANGED*)
  fun x ->
  fun n ->
    match n<1 with
      |true -> []
...
let padZero l1 l2 = let k = List.length l1 - List.length l2 in
    if(k<=0) then 
      if(k = 0) then 
        (l1,l2)
      else let y = clone 0 (-k) in 
          ((List.append y l1), l2)
    else 
      let z = clone 0 k in
        (l1,(List.append z l2))
...
let bigAdd l1 l2 = 
  let add (l1, l2) = 
    let f a x = match x with
      |(c,d) -> let y = c+d in
            match a with
              |(carry,(he::ta)) ->(carry,((he+y)/10)::((he+y)mod 10)::ta)
              |(carry,_) -> (carry, (y/10)::[y mod 10]) in
    let base = (0,[]) in
...
let rec mulByDigit i l = 
  let mul i l = 
    let f a x = let y = i*x in
        match a with
          |(carry,(h::t)) ->(carry,((h+y)/10)::((h+y)mod 10)::t)
          |(carry,_) -> (carry, (y/10)::[y mod 10]) in
    let base = (0,[]) in
...
let bigMul l1 l2 =
  ...
  let f a x  = (*match x with
                 |(x1,x2) -> let z = x1 in*)
    match a with
      |(carry, p) -> ((carry+1),(bigAdd p ((List.append (mulByDigit x l2) (clone 0 carry))))) in
  let base = (0, []) in
  let args = l1 in
  let(_,res) = List.fold_left f base args in
    res
...
match a with
|(carry, p) -> 
let y = bigAdd p (List.append (mulByDigit z l2) [0]) in
let base = (0, []) in
let args =  l1 in 
let (_, res) = List.fold_left f base args in
res*)
